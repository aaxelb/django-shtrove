...
class TestDigestiveTractExpel(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.indexcard_1 = create_indexcard(
            cls.focus_1, deriver_iris=[TROVE["derive/osfmap_json"]]
        )
        cls.indexcard_2 = create_indexcard(
            cls.focus_2, deriver_iris=[TROVE["derive/osfmap_json"]]
        )
    ...
    def setUp(self):
        ...
        self.mock_derive_task = self.enterContext(
            mock.patch("trove.digestive_tract.task__derive")
        )
        ...
        with mock.patch("trove.digestive_tract.expel_suid") as _mock_expel_suid:
            ...
            with self.captureOnCommitCallbacks(execute=True):
                digestive_tract.expel(
                    from_user=_user, record_identifier=self.suid_1.identifier
                )
    ...
    def test_expel_suid(self):
        ...
        self.assertIsNotNone(self.indexcard_2.latest_resource_description)
        self.assertEqual(
            self.indexcard_1.archived_description_set.count(), 1
        )  # not deleted
        self.assertEqual(self.indexcard_2.archived_description_set.count(), 1)
        self.assertEqual(
            self.indexcard_1.supplementary_description_set.count(), 1
        )  # not deleted
        self.assertEqual(self.indexcard_2.supplementary_description_set.count(), 0)
    ...
    def test_expel_supplementary_suid(self):
        ...
        self.assertEqual(self.indexcard_2.archived_description_set.count(), 1)
        self.assertEqual(
            self.indexcard_1.supplementary_description_set.count(), 0
        )  # deleted
        self.assertEqual(self.indexcard_2.supplementary_description_set.count(), 0)
        self.assertEqual(self.indexcard_1.derived_indexcard_set.count(), 1)
        self.assertEqual(self.indexcard_2.derived_indexcard_set.count(), 1)
    ...
    def test_expel_expired_task(self):
        with mock.patch(
            "trove.digestive_tract.expel_expired_data"
        ) as _mock_expel_expired:
            with self.captureOnCommitCallbacks(execute=True):
    ...
    def test_expel_expired(self):
        ...
        self.assertEqual(self.indexcard_1.archived_description_set.count(), 1)
        self.assertEqual(
            self.indexcard_2.archived_description_set.count(), 1
        )  # not deleted
        self.assertEqual(self.indexcard_1.supplementary_description_set.count(), 1)
        self.assertEqual(
            self.indexcard_2.supplementary_description_set.count(), 0
        )  # deleted
        self.assertEqual(self.indexcard_1.derived_indexcard_set.count(), 1)
