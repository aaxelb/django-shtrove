...
from shtrove.imps.shtrove import BasicShtrove
...
class DjangoShtrove(BasicShtrove):
    ...
    def each_index_imp(self) -> _abc.Iterator[ProtoIndex]:
        ...
        for _index_name, _index_settings in settings.SHTROVE_INDEXES.items():
            ...
            _index_constructor = load_entry_point(
                "shtrove.ProtoIndex", _entrypoint_name
            )
            yield _index_constructor(**_index_kwargs)

    def get_shtrove_search_imp(self, name: str = "") -> ProtoIndex:
        # TODO: error handling
