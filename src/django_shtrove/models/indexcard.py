...
class IndexcardManager(models.Manager["Indexcard"]):
    ...
    def save_indexcard_from_tripledict(
        ...
        *,
        ...
        _focus_identifier_set = (
            ResourceIdentifier.objects.save_equivalent_identifier_set(
                rdf_tripledict, focus_iri
            )
        )
        ...
        _indexcard.update_resource_description(
            focus_iri, rdf_tripledict, expiration_date=expiration_date
        )
        return _indexcard
    ...
    def supplement_indexcards(
        ...
        *,
        ...
        _indexcards = list(
            Indexcard.objects.filter(
                focus_identifier_set__in=ResourceIdentifier.objects.queryset_for_iri(
                    focus_iri
                ),
            )
        )
...
class Indexcard(models.Model):
    ...
    def pls_delete(self, *, notify_indexes: bool = True) -> None:
        ...
        (  # actually delete LatestResourceDescription:
            LatestResourceDescription.objects.filter(indexcard=self).delete()
        )
        (  # actually delete DerivedIndexcard:
            DerivedIndexcard.objects.filter(upriver_indexcard=self).delete()
        )
        if notify_indexes:
    ...
    def update_resource_description(
        ...
        if focus_iri not in rdf_tripledict:
            raise DigestiveError(
                f"expected {focus_iri} in {set(rdf_tripledict.keys())}"
            )
        _rdf_as_turtle, _turtle_checksum_iri = _turtlify(rdf_tripledict)
        _archived, _archived_created = (
            ArchivedResourceDescription.objects.get_or_create(
                indexcard=self,
        ...
        if not self.deleted:
            _latest_resource_description, _created = (
                LatestResourceDescription.objects.update_or_create(
                    indexcard=self,
                    defaults={
                        "turtle_checksum_iri": _turtle_checksum_iri,
                        "rdf_as_turtle": _rdf_as_turtle,
    ...
    def update_supplementary_description(
        ...
        _rdf_as_turtle, _turtle_checksum_iri = _turtlify(rdf_tripledict)
        _supplementary_rd, _ = (
            SupplementaryResourceDescription.objects.update_or_create(
                indexcard=self,
                supplementary_record_identifier=supplementary_record_identifier,
                defaults={
                    "turtle_checksum_iri": _turtle_checksum_iri,
