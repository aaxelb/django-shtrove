...
class ResourceIdentifier(models.Model):
    ...
    # using IP or DNS or similar), the substring starting with "://" is "sufficiently unique"
    # -- otherwise, use the full IRI
    suffuniq_iri = models.TextField(
        unique=True,
    ...
    # original IRIs that were reduced to `suffuniq_iri`
    ...
    def __repr__(self) -> str:
        return f'<{self.__class__.__qualname__}({self.pk}, "{self.suffuniq_iri}")'
    ...
    def find_equivalent_iri(self, tripledict: primitive_rdf.RdfTripleDictionary) -> str:
        ...
        for _iri, _twopledict in tripledict.items():
            ...
            if _is_equivalent:
                return _iri
        raise shtrove_exceptions.IriMismatch(
            f'could not find "{_identifier_iri}" or equivalent in {set(tripledict.keys())}'
        )
